...
public class TopicConsumer {
    ...
    private void processTopicMessage(TopicMessage topicMessage) {
        try {
            ...
            // Classify the topic content
            List<ThemeClassification> classifications = classificationService.classifyContent(
                    topicMessage.getTitle(), 
                    topicMessage.getContent()
            );
            
            // Save theme classifications
            for (ThemeClassification classification : classifications) {
                topicService.addTopicTheme(topic, classification.getTheme(), 
                        classification.getConfidenceScore());
            }
            
            logger.info("Processed topic: {} with {} theme classifications", 
                    topicMessage.getTitle(), classifications.size());
