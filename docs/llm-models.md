...
|------|---------|-------------------|----------|-------------------|
| **Relevance** | Score topic relevance to categories (0.0–1.0) | `qwen2.5:14b` | ~10 GB | `gpt-4o-mini` |
| **Classification** | Categorize topics into themes | `qwen2.5:14b` | ~10 GB | `gpt-4o-mini` |
| **Summarization** | Summarize extracted content | `qwen2.5:32b` | ~20 GB | `gpt-4o` |
| **Embedding** | Generate vector embeddings for dedup | `nomic-embed-text` | ~0.5 GB | `text-embedding-3-small` |
| **Generation** | Generate blog posts, scripts, newsletters | `qwen2.5:32b` | ~20 GB | `gpt-4o` |
...
Pull all 3 models (qwen2.5:14b, qwen2.5:32b, nomic-embed-text):
...
```bash
# Test chat model
ollama run qwen2.5:14b "What is Kubernetes?"
...
- `qwen2.5:14b`: Usable (~10-30s per response)
- `qwen2.5:32b`: Very slow (~60-120s per response) — recommend cloud fallback
...
| Summarization | `LLM_OLLAMA_MODEL_SUMMARIZATION` | `llama3.1:70b`, `qwen2.5:72b`, `mixtral:8x7b` |
| Embedding | `LLM_OLLAMA_MODEL_EMBEDDING` | `mxbai-embed-large`, `all-minilm` |
| Generation | `LLM_OLLAMA_MODEL_GENERATION` | `llama3.1:70b`, `qwen2.5:72b`, `mixtral:8x7b` |
...
- **Quantization**: Ollama models are quantized (Q4_K_M by default). For better quality at the cost of more VRAM, use `qwen2.5:14b-instruct-fp16`.
...
- Use smaller models: replace `qwen2.5:32b` with `qwen2.5:14b` for summarization/generation
